...
$products_data = [];
try {
    $products_data = search_products_paginated_ctr($search_query, $limit, $offset);

    // Apply additional filters if specified
    if ($products_data['success'] && !empty($products_data['products'])) {
        $all_products = $products_data['products'];

        // Filter by category if specified
        if ($selected_category > 0) {
            $all_products = array_filter($all_products, function($product) use ($selected_category) {
                return $product['product_cat'] == $selected_category;
            });
        }

        // Filter by brand if specified
        if ($selected_brand > 0) {
            $all_products = array_filter($all_products, function($product) use ($selected_brand) {
                return $product['product_brand'] == $selected_brand;
            });
        }

        $all_products = array_values($all_products); // Reindex array
...
<head>
    ...
    <title>Search Results: "<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>" - Taste of Africa</title>
    ...
    <div class="page-header">
        <div class="container text-center">
            ...
            <p class="animate__animated animate__fadeInUp">
                Showing results for: <strong>"<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>"</strong>
            </p>
        </div>
    </div>
            ...
            <form method="GET" action="product_search_result.php">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4 d-flex align-items-end gap-2">
                        ...
                        <a href="product_search_result.php?query=<?php echo urlencode($search_query); ?>" class="filter-btn" style="background: #6c757d; text-align: center; text-decoration: none;">
